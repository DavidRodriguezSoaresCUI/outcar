...
public class SDLActivity extends Activity {
    ...
    protected String[] getArguments() {
         ...
         * 2: String, Sex
         * 3: String, Handedness
         * 4: String, file location for stat database
         * 5: String, file location for game config
         */
        ...
        String pers_info_file =  pers_info_file_folder + File.separator
                    + Res.getString( R.string.pers_info_file );
        String stat_db_file_unformatted = pers_info_file_folder + File.separator
                    + Res.getString( R.string.stat_db_file );
        String game_config_file = pers_info_file_folder + File.separator
        ...
        String stat_db_file = String.format(stat_db_file_unformatted, android_id);
        ...
        try
        {
            ...
            if ( file.exists() ) {
                try {
                    BufferedReader reader = new BufferedReader( new FileReader( pers_info_file ) );
                    try {
                        String line;
            ...
            File cruiserDB = new File( stat_db_file );
            if ( !cruiserDB.exists() ){
                // Creates a file if it doesn't exist, with exception control
                try {
                    if ( cruiserDB.createNewFile() )
                    {
                        // the file was just created, so we write the first line
                        PrintWriter writer = new PrintWriter(new FileWriter( cruiserDB, true));
                        writer.print( Res.getString( R.string.stat_db_file_1stline ) );
                        writer.append('\n');
                        writer.close();
                    }
                } catch (IOException e) {
